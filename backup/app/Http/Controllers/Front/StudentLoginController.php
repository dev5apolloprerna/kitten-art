...
class StudentLoginController extends Controller
{
    ...
    public function studentlogin(Request $request)
    {
        ...
        if ($user) 
        {
            ...
            if (Auth::guard('student')->attempt($credentials)) 
            {
                ...
                $user=Student::select('student_first_name','student_id','email','mobile')->where(['student_id'=>$user->student_id,'isWaiting'=>0,'isRegister'=>1,'isPaid'=>1,'iStatus'=>1])->first();
                if(!empty($user))
                {
    ...
    public function forgotpasswordsubmit(Request $request)
    {
        ...
        if (!empty($Student)) {
            ...
            $file = file_get_contents($root . '/kitten_craft/mailers/forgetpassword.html', 'r');
            $file = str_replace('#parent_name', $Student->parent_name, $file);
            $file = str_replace('#email', 'https://getdemo.in/kitten_craft/New-Password/' . $token, $file);
            // dd($file);
