...
class FrontController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('frontview.index',compact('category','plan','batch','Testimonials','gallery','about','class','image','banners'));
        } catch (\Exception $e) {
    ...
    public function contactStore(Request $request)
    {
         ...
         $request->validate([
                ...
                'student_first_name.required' => 'first name is required.',
                'student_last_name.required' => 'last name is required.',
                'student_age.required' => 'Please enter the student\'s age.',
                'student_age.integer' => 'The age must be a number.',
                'mobile.required' => 'Mobile number is required.',
                'mobile.digits' => 'Mobile number must be 10 digits.',
                'email.required' => 'Email  is required.',
                'message.required' => 'Please select a communication mode.',
            ]);
            ...
            //$batch=Batch::where(['batch_id'=>$request->batch_id])->first();
                ...
                $SendEmailDetails = DB::table('sendemaildetails')->where(['id' => 3])->first();
                    ...
                    $msg = array(
                        ...
                        $mail = Mail::send('emails.inquiry', ['data' => $data1], function ($message) use ($msg) {
                    ...
                    $data2=array(
                        ...
                        $mail = Mail::send('emails.adminmail1', ['data' => $data2], function ($message) use ($adminmsg) {
            ...
            //$batch=Batch::where(['batch_id'=>$request->batch_id])->first();
                ...
                $SendEmailDetails = DB::table('sendemaildetails')->where(['id' => 3])->first();
                    ...
                    $data1=array(
                        ...
                        $mail = Mail::send('emails.adminmail2', ['data' => $data1], function ($message) use ($adminmsg) {
                            $message->from($adminmsg['FromMail'], $adminmsg['Title']);
                            $message->to($adminmsg['ToEmail'])->subject($adminmsg['Subject']);
                        });
...
*/
    ...
    public function service_gallery($id)
    {
        ...
        $service=Service::where(['slug'=>$id])->orderBy('service_id', 'DESC')->first();
        if($service)
        {
    ...
    public function renewal_thank_you(){
        return view('frontview.renewal_thank_you');
    }
     ...
     public function privacypolicy()
    {
        ...
        return view('frontview.privacypolicy', compact('data'));
    }
    public function termandcondition()
    {
 
        $data = Page::where('id', '=', '3')->first();
        return view('frontview.termandcondition', compact('data'));
    }
    public function supply_list()
    {
 
        return view('frontview.supply_list');
    }
    public function payment_information()
    {
 
        return view('frontview.payment_information');
    }
}
