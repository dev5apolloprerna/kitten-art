...

                {
...

                    {
                        ...
                        if ($ledger->debit_balance > 0) {
                            ...
                            $new_debit_balance =  $revert_session; // Reduce the debit balance
            ...
            if($status == 'P')
            {
                 ...
                 StudentAttendance::where("attendence_id","=",$request->attendence_id)->update($data);
                    ...
                    if ($ledger && $ledger->closing_balance != 0) 
                    {
                        ...
                        $ledgerData =new StudentLedger();
                            $ledgerData->student_id = $student->student_id;
                            $ledgerData->attendence_id = $student->sattendanceid;
                            $ledgerData->attendence_detail_id = $student->attendence_id;
                            $ledgerData->opening_balance = $new_opening_balance;
                            $ledgerData->credit_balance = $new_credit_balance;
                            $ledgerData->debit_balance = $new_debit_balance;
                            $ledgerData->closing_balance = $new_closing_balance;
                        $ledgerData->save();
        ...
        if($status == 'A')
        {
                 ...
                 $ledger = StudentLedger::where('student_id', $studentId)->latest()->first();
                    /*if ($ledger) 
                    {
                        ...
                        // Define the amount to revert
                        if ($ledger->credit_balance > 0) 
                        {
                            StudentLedger::where('ledger_id','=',$ledger->ledger_id)->delete();
                        }if($ledger->debit_balance > 0)
                        {
                            StudentLedger::where('ledger_id','=',$ledger->ledger_id)->delete();
                        }
...

                    {
                        ...
                        $data=array(
                            ...
                            // $file = file_get_contents(public_path('mailers/cancel_subsctiption.html'));

                            // $file = str_replace('#parent_name', $s->parent_name, $file);

                            // $file = str_replace('#url', 'https://kittenart.com/newkittenart/renew-subscription/' . $studentId, $file);
