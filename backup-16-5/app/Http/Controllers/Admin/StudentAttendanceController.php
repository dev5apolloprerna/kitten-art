   ...
   public function markAttended(Request $request)
    {
        ...
        if(sizeof($attendance) == 0)
        {
            foreach ($studentIds as $studentId) 
            {
                ...
                $attendance=StudentAttendance::where('student_id', $studentId)->whereDate('student_attendance.created_at', '=', $today)
                ->join('student_attendance_master', 'student_attendance_master.sattendanceid', '=', 'student_attendance.sattendanceid')->latest()->first();
                if($attendance)
                {
                    $attendanceData = [
                ...
                if ($ledger && $ledger->closing_balance != 0) 
                {
                    ...
                    if($attendance)
                    {
                        ...
                        $ledgerData = [
                            'attendence_id' => $attendance->attendence_id ?? null,
                            'sattendanceid' => $attendance->sattendanceid ?? null,
                            'subscription_id' => $ledger->subscription_id ?? null,
                            'student_id' => $studentId,
...
//                         StudentLedger::create($ledgerData);
    ...
    try{        
            ...
            {
              return view('admin.student_attendance.student_view',compact('data','subscription','attendance','debit_balance','credit_balance'));
            }
        } catch (\Exception $e) {
                return redirect()->back()->with('error', 'An error occurred: ' . $e->getMessage());
        }
