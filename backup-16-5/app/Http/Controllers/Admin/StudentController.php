   ...
   public function index(Request $request)
    ...
    public function inactive_student(Request $request)
    {
        try
        {
             $Student = Student::select('student_master.*', 
                ...
                ->where(['isWaiting'=>0,'isRegister'=>0,'isPaid'=>0])
                ->join('batch_master', 'batch_master.batch_id', '=', 'student_master.batch_id')
    ...
    public function active_student_view($id)
    {
        ...
        if(empty($data))
        {
            ...
            return view('admin.student.active_student_view',compact('data','subscription','attendance','debit_balance','credit_balance'));
        }
            ...
            $SendEmailDetails = DB::table('sendemaildetails')->where(['id' => 4])->first();
                ...
                $data=array(
                 ...
                 $msg = array(
                    ...
                    'Title' => 'Kitten Art Classes',
                    'ToEmail' => $student->email,
                    'Subject' => 'Thank You For Your Payment With Kitten Art Classes'
                );
