...

    {
         ...
         try{
                ...
                $Student = Student::select('student_master.*', 
                    ...
                    DB::raw('(select sum(credit_balance) from student_ledger where student_ledger.student_id = student_master.student_id and student_ledger.subscription_id !=0 AND student_ledger.attendence_id = 0 AND student_ledger.attendence_detail_id = 0 order by ledger_id desc limit 1) as credit_balance'),
                    DB::raw('(
                        SELECT 
                            SUM(debit_balance) - 
                            SUM(CASE 
                            ...
                            END)
                        FROM student_ledger 
                        WHERE student_ledger.student_id = student_master.student_id
                    ) AS debit_balance'),                    DB::raw('(select closing_balance from student_ledger where student_ledger.student_id = student_master.student_id order by ledger_id desc limit 1) as closing_balance')
...

    {
        ...
        try{
            ...
            $subscription = StudentLedger::select(
                'student_ledger.*','student_subscription.amount','student_subscription.activate_date','student_subscription.subscription_id',
            ...
            ->join('student_subscription', 'student_subscription.subscription_id', '=', 'student_ledger.subscription_id')->where(['student_ledger.student_id'=>$id,'attendence_id'=>0,'attendence_detail_id'=>0])
            ...
            {
                return view('admin.report.upcoming_view',compact('data','subscription','attendance','debit_balance','credit_balance'));
            }
        } catch (\Exception $e) {
